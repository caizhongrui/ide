...
| 6 | **会话自动命名（F13）**（首条消息启发式生成标题，码弦特有 UX，九思无此功能） | F13 实现（v0.2.x） | `apps/desktop/src/sessionAutoName.ts` → 九思 desktop 同位置（纯函数模块原样拷贝）+ 触发点插九思 App.tsx 的 `send()` | 设计文档：`docs/architecture/session-auto-name.md`。⚠️ 九思的 createSession 默认标题格式需排查后补进 `isAutoGeneratedTitle` 白名单；`renameSession`（PATCH /sessions/{id}）SDK 同源应已有，确认即可 |
