...
export default function App() {
  ...
  async function send() {
    ...
    // F13: 首条消息时自动命名（纯前端启发式，参考 Claude Code / Codex 的命名策略）。
    // 仅在 messages 为空 + 当前 session.title 是自动生成的默认名（"新会话"/"会话 HH:MM"/"对话 HH:MM" 等）
    // 时才覆盖；用户手改过的 title 保留不动。fire-and-forget — 失败静默忽略，不影响发消息主流程。
    if (messages().length === 0) {
      void (async () => {
        try {
          const session = sessions().find((s: any) => s.id === sid) as any
          const { smartTitleFromMessage, isAutoGeneratedTitle } = await import('./sessionAutoName')
          if (session && isAutoGeneratedTitle(session.title)) {
            const title = smartTitleFromMessage(content)
            if (title && !isAutoGeneratedTitle(title)) {
              const c = await getClient()
              await c.renameSession(sid, title)
              await refreshSessions()
            }
          }
        } catch (e) {
          console.warn('[autoName] 自动命名失败（忽略，不影响发消息）:', e)
        }
      })()
    }
