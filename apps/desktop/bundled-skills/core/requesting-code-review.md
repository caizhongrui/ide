...
> ⚠️ **码弦项目提示**:码弦是**单一 git 仓库的 pnpm monorepo**(根目录即仓库根)。
> - `git push` 和 `gh pr create` 在仓库根目录执行
> - 改动即使跨多个包,**一个 PR** 即可涵盖
...
> **提示**:对于多文件 / 多模块的 PR,可以先调用 `todo_write` 工具把下面 A-E 五段建成 todo 列表,挨个勾选完成,前端会实时显示进度。
